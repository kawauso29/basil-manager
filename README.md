...
QRコードは**株を作った時点から**、管理画面の株詳細に表示されます。準備のための
コマンド実行は不要です。

出荷分をまとめて刷るときは、株一覧で絞り込んでから **「QRラベルを印刷」** を押します。
一覧と同じ絞り込みが効いたA4のラベルシート（60×42mmのカードが3列×6行＝18枚／ページ）
が開くので、ブラウザから等倍で印刷してください。破線が切り取り線です。

画像は `GET /admin/stocks/:id/qr` がその都度描いて返します。中身は `public_token` から
決まるため保存しません。保存すると、公開ドメインを変えたときに古い画像が残ります。
ラベルシートでは、印刷時に画像が未読み込みで欠けるのを避けるため、PNGをHTMLへ
直接埋め込んでいます。
