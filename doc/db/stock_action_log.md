...
| `created_at` | `datetime` | 不可 | なし | なし | 登録日時 |
...
- `action_type`は`seed_sown`、`cutting_started`、`water_started`、`watered`、
  `fertilized`、`pinched`、`pruned`、`water_replaced`、`harvested`、`moved`、
  `status_changed`、`transplanted`のいずれかとする
- `moved`と`status_changed`はStockの専用操作から作成し、通常の作業ログ
  作成画面では選択しない
- `moved`は`from_location_id`と`to_location_id`、`status_changed`は
  `status_before`と`status_after`を必ず記録する
...
- `moved`と`status_changed`は現在値の更新を伴うため、一括記録の
  `action_type`には使用しない
- 一括記録の対象は育成中かつ指定した環境・Locationに一致するStockに限り、
  画面の絞り込みとは別にサーバー側でも検証する
- 一括作成はトランザクション内で行い、1件でも保存に失敗した場合は全件を
  ロールバックする
